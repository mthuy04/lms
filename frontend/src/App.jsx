...
const useAppState = () => {
  ...
  const login = useCallback((userData, jwtToken) => {
    ...
    // SỬA: Phân tuyến chính xác sau khi login
    setRoute(userData.role === 'admin' ? 'admin_dashboard' : 'books');
  }, []);
    ...
    <header className="bg-gray-900 shadow-xl text-white sticky top-0 z-40">
      <div className="container mx-auto px-4 py-4 flex justify-between items-center">
        ...
        <nav className="flex items-center space-x-4">
          ...
          {user ? (
            <>
              {/* SỬA: Link 'Books' trỏ đến route 'books' */}
  ...
  return (
    <Card title={title} className="max-w-md mx-auto my-12">
      ...
      <p className="mt-4 text-center text-sm text-gray-500">
        ...
        <button onClick={() => appState.setRoute(isLogin ? 'register' : 'login')} className="text-indigo-600 hover:text-indigo-800 font-medium">
...
const DashboardPage = ({ appState }) => {
  ...
  // SỬA: Xóa bỏ 'sampleBooks'
  
  const fetchBooks = useCallback(async () => {
    setLoading(true);
    setError(null);
    try {
      // SỬA: Bỏ sample data, dùng API call thật
      const res = await axios.get('/books');
      setBooks(Array.isArray(res.data) ? res.data : []);
      
    } catch (error) {
      console.error('Error fetching books:', error);
      // SỬA: Bỏ fallback sample data
      const errorMessage = error.response?.data?.msg || 'Failed to fetch books from backend.';
      setError(errorMessage);
      notify(errorMessage, 'error');
    } finally {
      setLoading(false);
    }
  }, [setLoading, notify]); // SỬA: Bỏ 'token' khỏi dependency

  useEffect(() => {
    // SỬA: Luôn luôn fetch books, không cần kiểm tra token
    console.log('Fetching books...');
    fetchBooks();
  }, [fetchBooks]); // SỬA: Bỏ 'token'
  ...
  const handleBorrow = async (bookId, title) => {
    ...
    try {
        ...
        notify(`Successfully borrowed "${title}"!`, 'success');
        
        // SỬA: Gọi fetchBooks() để tải lại danh sách
        fetchBooks(); 
          ...
          {books.map((book) => (
            <Card key={book.id} className="flex flex-col justify-between">
              <div>
                ...
                <p className="text-xs mt-1 text-gray-500">Year: {book.published_year}</p>
              </div>
...
const App = () => {
  ...
  const renderContent = useMemo(() => {
    switch (appState.route) {
      ...
      // SỬA: Thêm route 'books'
      case 'books':
        return <DashboardPage appState={appState} />;

      // SỬA: Đổi tên 'dashboard' thành 'admin_dashboard'
      case 'admin_dashboard':
        return appState.user?.role === 'admin' ? (
          <AdminDashboard appState={appState} />
        ) : (
          <DashboardPage appState={appState} /> // Người dùng thường sẽ thấy sách
        );
  ...
  }, [appState.route, appState]); // Thêm appState vào dependency array
    ...
    <div className="container mx-auto px-4 py-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <DashboardCard
          ...
          // SỬA: Điều hướng đến 'books'
          onClick={() => appState.setRoute('books')} 
        />
        <DashboardCard
          title="Manage Users"
          icon={<User className="h-8 w-8" />}
          onClick={() => appState.notify('User management coming soon!', 'success')}
        />
        <DashboardCard
          title="Settings"
          icon={<Settings className="h-8 w-8" />}
          onClick={() => appState.notify('Settings coming soon!', 'success')}
        />
      </div>
    </div>
  );
...
const DashboardCard = ({ title, icon, onClick }) => {
  return (
    <button
      onClick={onClick}
      className="p-6 bg-white rounded-lg shadow hover:shadow-md transition-shadow duration-200 text-left w-full hover:bg-indigo-50 border-2 border-transparent hover:border-indigo-300"
    >
